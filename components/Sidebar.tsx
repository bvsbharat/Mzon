...
const NavItem: React.FC<NavItemProps> = ({ icon, label, isActive, onClick, isCollapsed = false }) => {
    const buttonContent = (
        <button
            onClick={onClick}
            className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} px-3 py-2.5 rounded-lg text-sm transition-colors duration-200 ease-in-out ${
                isActive
...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, credits, isOpen, onClose }) => {
    ...
    return (
        <>
            {/* Backdrop for mobile */}
            <div 
                className={`fixed inset-0 bg-black/40 z-40 transition-opacity md:hidden ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                onClick={onClose}
                aria-hidden="true"
            ></div>
            <aside
                className={`fixed inset-y-0 left-0 z-50 ${isCollapsed ? 'w-16' : 'w-64'} bg-white flex flex-col border-r border-gray-200
            ...
            >
                ...
                <div className="flex items-center justify-between shrink-0 px-4 h-16">
                    ...
                    >
                        <Icon icon={isCollapsed ? "chevronsRight" : "chevronsLeft"} className="w-4 h-4" />
                    </button>
                    {/* Empty div for spacing on mobile, so close button goes right */}
                    <div className="md:hidden" />
                    <button onClick={onClose} className="md:hidden -mr-2 p-2 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200" aria-label="Close menu">
                ...
                <nav className="flex-grow p-4 overflow-y-auto space-y-4">
                    <div>
                        {!isCollapsed && (
                            <h2 className="px-3 text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-2">Playground</h2>
                        )}
                    ...
                    <div className="pt-4 border-t border-gray-200">
                        {!isCollapsed && (
                            <h2 className="px-3 text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-2">Social Media</h2>
                        )}
                        <div className="space-y-1">
                            <NavItem
                    ...
                    <div className="pt-4 border-t border-gray-200">
                        {!isCollapsed && (
                            <h2 className="px-3 text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-2">Campaigns</h2>
                        )}
                        <div className="space-y-1">
                           <NavItem
                ...
                <div className="flex-shrink-0 mt-auto p-4 border-t border-gray-200">
                    <div className={`px-3 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
                        {isCollapsed ? (
                    ...
                    {!isCollapsed && (
                        <div className="hidden md:block pt-4">
                            <div className="relative" ref={menuRef}>
                                {isMenuOpen && (
                                    <div
                                        className="absolute bottom-full left-0 right-0 mb-2 w-full origin-bottom rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-10"
                                    >
                                        <div className="py-1">
                                            <button
                                ...
                                >
                                    ...
                                    <Icon icon="chevronsUpDown" className="w-5 h-5 text-gray-400" />
                                </button>
                            </div>
                        </div>
                    )}
                    {isCollapsed && (
                        <div className="hidden md:block pt-4">
                            <Tooltip content={user?.email || 'user@example.com'} position="right">
