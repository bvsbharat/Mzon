...
const SocialPostButton: React.FC<SocialPostButtonProps> = ({
  platform,
  content,
  hashtags,
  mediaUrls,
  ...
  const handlePostNow = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setPostStatus('error');
        onPostError?.(result.error || 'Failed to post content');
      }
    } catch (error) {
      setPostStatus('error');
  ...
  const getButtonText = () => {
    ...
    if (postStatus === 'success') return 'Posted!';
    if (postStatus === 'error') return 'Failed';
    return `Post to ${platform.charAt(0).toUpperCase() + platform.slice(1)}`;
  };
  ...
  const getButtonColor = () => {
    ...
    switch (platform) {
      ...
      case 'instagram':
        return 'bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 border-purple-500';
      case 'facebook':
  ...
  return (
    <div className="relative">
      <button
        ...
        disabled={disabled || isPosting || !isAiriaConfigured}
        className={`
          flex items-center space-x-2 px-4 py-2 text-white text-sm font-medium rounded-lg border transition-all duration-200
          ${getButtonColor()}
          ${disabled || isPosting || !isAiriaConfigured ? 'opacity-50 cursor-not-allowed' : ''}
          ${className}
        `}
        title={!isAiriaConfigured ? 'Airia AI agent not configured' : ''}
      >
        {isPosting ? (
      ...
      {!isAiriaConfigured && (
        <div className="absolute top-full left-0 mt-1 text-xs text-red-600 whitespace-nowrap">
          Configure Airia API to enable posting
        </div>
      )}
    </div>
  );
